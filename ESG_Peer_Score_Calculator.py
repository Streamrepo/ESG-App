...
if uploaded_file:
    try:
        ...
        if all(col in df.columns for col in required_columns):
            ...
            E_score = E_raw * (60 / (E_count * 100 + 100))
            ...
            # --- Section 5: ESG Score vs Expected Return ---
            st.header("5. ESG Score vs Expected Return")
            benchmark_path = f"data/benchmarks/{industry.lower().replace(' ', '_')}/{size}/esg_return_benchmark.csv"

            if os.path.exists(benchmark_path):
                try:
                    esg_benchmark_df = pd.read_csv(benchmark_path)

                    def logistic(x, a, b, c):
                        return a / (1 + np.exp(-b * (x - c)))

                    X = esg_benchmark_df["Expected Return (%)"].values
                    y = esg_benchmark_df["ESG Score"].values
                    ...
                    your_return = company["Expected Return (%)"]
                    your_score = total_score

                    fig, ax = plt.subplots()
                    fig.patch.set_facecolor('#999999')
                    ax.set_facecolor('#cccccc')
                    ax.scatter(X, y, label="Peer Benchmark", alpha=0.6)
                    ax.plot(x_fit, y_fit, color="#ffe600", label="Logistic Fit")
                    ax.scatter([your_return], [your_score], color="red", s=80, label="Your Company", zorder=5)
                    ...
                    st.markdown(f"**Your Expected Return:** {your_return:.2f}%")
                    st.markdown(f"**Your ESG Score:** {your_score:.2f}")

                except Exception as e:
                    st.error(f"⚠️ Error creating ESG vs Return plot: {e}")
            else:
                st.warning(f"⚠️ Benchmark file not found at: `{benchmark_path}`")
